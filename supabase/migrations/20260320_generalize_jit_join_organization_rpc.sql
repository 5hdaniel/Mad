-- TASK-2304: Generalize jit_join_organization RPC for multi-provider support
-- Sprint: SPRINT-L (Provider Abstraction & SCIM Decomposition)
--
-- Replaces the Azure-only jit_join_organization(p_tenant_id TEXT) with a
-- multi-provider version: jit_join_organization(p_provider_type TEXT, p_identifier TEXT)
--
-- Supported providers:
--   azure_ad         -> looks up by microsoft_tenant_id
--   google_workspace -> looks up by google_workspace_domain
--
-- IMPORTANT: Different parameter count = new function in PostgreSQL (overloading).
-- We must explicitly DROP the old 1-param version to avoid ambiguity.

-- Step 1: Drop the old 1-param overload
DROP FUNCTION IF EXISTS public.jit_join_organization(TEXT);

-- Step 2: Create the new 2-param version
CREATE OR REPLACE FUNCTION public.jit_join_organization(
...
BEGIN
  ...
  IF p_provider_type NOT IN ('azure_ad', 'google_workspace') THEN
    RETURN jsonb_build_object('success', false, 'error', 'unsupported_provider');
  END IF;

  -- Provider-specific logic
  IF p_provider_type = 'azure_ad' THEN
    ...
    IF p_identifier = '9188040d-6c67-4c5b-b112-36a304b66dad' THEN
    ...
    FROM organizations
    WHERE microsoft_tenant_id = p_identifier;

    v_oauth_provider := 'azure';

  ELSIF p_provider_type = 'google_workspace' THEN
    -- Look up organization by google_workspace_domain
    SELECT id, COALESCE(default_member_role, 'agent')
    INTO v_org_id, v_org_role
    FROM organizations
    WHERE google_workspace_domain = p_identifier;

    v_oauth_provider := 'google';
  END IF;
  ...
  -- Provider may not populate auth.users.email for external tenants
  SELECT
    COALESCE(
      email,
  ...
  VALUES (v_user_id, v_user_email, v_oauth_provider, v_oauth_id, 'jit', true, NOW())
