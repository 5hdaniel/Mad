-- BACKLOG-1183: Add p_unassigned_only parameter to pm_list_items
-- When TRUE, only returns items with sprint_id IS NULL (backlog panel).
-- Similar pattern to existing p_root_only parameter.

DROP FUNCTION IF EXISTS pm_list_items(
  text, text, text, text, uuid, uuid, text, uuid[], uuid, integer, integer, uuid, boolean
);
...
BEGIN
  ...
  WITH filtered_items AS (
    ...
    WHERE i.deleted_at IS NULL
      ...
      AND (NOT p_unassigned_only OR i.sprint_id IS NULL)
  )
