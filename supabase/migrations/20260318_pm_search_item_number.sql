-- ============================================
-- Fix: Add item_number matching to PM backlog search
--
-- Problem: pm_list_items and pm_search_items_for_link only match on
-- search_vector, title, legacy_id, and comments. Items created directly
-- in Supabase have no legacy_id, so searching by number (e.g. "1050")
-- returns nothing.
--
-- Fix: Add item_number matching:
--   1. Partial text match: item_number::text ILIKE '%<search>%'
--   2. Exact numeric match: item_number = <search>::int (when search is purely digits)
-- ============================================


-- ============================================
-- pm_list_items: Add item_number to search conditions
-- ============================================
...
GRANT EXECUTE ON FUNCTION pm_list_items(TEXT, TEXT, TEXT, TEXT, UUID, UUID, TEXT, UUID[], UUID, INT, INT) TO authenticated;


-- ============================================
-- pm_search_items_for_link: Add item_number to search conditions
-- ============================================
...
GRANT EXECUTE ON FUNCTION pm_search_items_for_link(TEXT, UUID) TO authenticated;
