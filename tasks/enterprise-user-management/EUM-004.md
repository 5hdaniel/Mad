...
- ❌ Custom role creation (fixed roles only: owner, admin, member)
- ❌ Resource-level permissions (e.g., access to specific projects)
- ❌ Permission groups or composite roles
...
- [ ] `getUserRole()` returns correct role or null for non-members
- [ ] `hasPermission()` correctly checks role against permission matrix
- [ ] `hasPermissions()` batch-checks multiple permissions efficiently
- [ ] `getUserPermissions()` returns all permissions for user's role
- [ ] `isAdminOrOwner()` returns true only for admin/owner roles
- [ ] `isOwner()` returns true only for owner role
- [ ] `canManageUser()` enforces role hierarchy correctly
- [ ] `canAssignRole()` prevents owner role assignment (only transfer)
- [ ] `requirePermission()` throws clear error when denied
...
1. **Permission matrix changes** - If you believe any permission should be added/removed/moved between roles
2. **Role hierarchy questions** - If the admin-can-manage-admin rule seems wrong for any scenario
3. **Owner protection** - If any operation seems like it should be able to target the owner
4. **Suspended user handling** - If you're unsure how suspended users should be handled in permission checks
5. **Caching needs** - If you believe role lookups need caching for performance
6. **Custom permissions** - If any use case requires permissions not in the matrix
...
- Role is stored in `organization_members.role` column
- Status must be 'active' for permissions to apply

### For EUM-002 (Organization Service)
- Organization handlers should use RBAC checks before operations
- Use `withPermission()` middleware or `checkPermissionAndExecute()` helper

### For EUM-003 (Audit Service)
- Role changes should be logged with before/after details
- Permission denied events could optionally be logged

### For EUM-005 (Integration Task)
- RBAC service must be a singleton
- Permission constants should be exported for use in handlers
- `withPermission()` middleware wraps protected handlers

### For Phase 2+ (SSO, Admin Portal)
- UI should use `getPermissions()` to show/hide features
- SSO configuration requires admin or owner role
