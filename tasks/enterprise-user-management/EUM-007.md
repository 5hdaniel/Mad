...
- ❌ Modifying existing individual Google OAuth login
...
- ❌ Implementing SCIM provisioning with Google Workspace
- ❌ Google Cloud Directory Sync
- ❌ Supporting personal Gmail accounts via SSO
...
- [ ] `getAuthorizationUrl()` generates valid OAuth2 URL with PKCE and `hd` parameter
- [ ] `exchangeCodeForTokens()` exchanges code for valid tokens
- [ ] Token claims are correctly parsed (sub, email, hd, given_name, family_name)
- [ ] `verifyEmailDomain()` correctly validates email domain
- [ ] Hosted domain (hd) claim is verified against configuration
- [ ] Email verification status (email_verified) is checked
- [ ] `testConfiguration()` validates OAuth client configuration
- [ ] `getUserInfo()` retrieves user profile from Google userinfo endpoint
...
1. **googleapis library issues** - If `googleapis` has breaking changes or conflicts
2. **Token claims differences** - If Google ID token claims don't match expected structure
3. **Workspace-specific APIs** - If Directory API access is needed for groups
4. **OAuth consent screen** - If additional consent screen configuration is required
5. **Internal vs external apps** - If app type configuration affects behavior
6. **Domain verification** - If Google domain verification process differs from expected
...
- SSO Core will call this service for Google Workspace authentication
- Must decrypt client secret before use
- Return standard `GoogleWorkspaceSSOResult` format

### For EUM-009 (JIT Provisioning)
- Provide `GoogleWorkspaceUserInfo` structure compatible with JIT service
- Include domain info for verification
...
- The `hd` parameter restricts to Workspace domains only
