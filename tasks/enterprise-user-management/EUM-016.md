...
- ❌ SCIM Groups endpoint (Users only for now)
- ❌ Custom SCIM extensions
- ❌ SCIM bulk operations endpoint
- ❌ Self-service SCIM registration
- ❌ Real-time webhook notifications
- ❌ SCIM event streaming
...
- ✅ Return proper SCIM schemas in all responses
- ✅ Validate bearer tokens on every request
- ✅ Log all provisioning events to audit log
- ✅ Support standard SCIM filtering (userName eq "...")
- ✅ Return proper HTTP status codes per SCIM spec

## Don'ts

- ❌ Don't return 200 for errors (use proper SCIM error format)
- ❌ Don't hard-delete users (use status: suspended)
- ❌ Don't expose internal user IDs (use stable external IDs)
- ❌ Don't process requests with invalid tokens

## Acceptance Criteria

- [ ] ServiceProviderConfig endpoint returns capabilities
- [ ] Schemas endpoint returns User schema
- [ ] GET /Users returns paginated user list
- [ ] GET /Users/:id returns specific user
- [ ] GET /Users?filter=userName%20eq%20"..." works
- [ ] POST /Users creates new user and org membership
- [ ] PUT /Users/:id replaces user data
- [ ] PATCH /Users/:id updates specific attributes
- [ ] DELETE /Users/:id suspends user (not hard delete)
- [ ] Bearer token authentication works
- [ ] Invalid token returns 401
- [ ] Duplicate user returns 409
- [ ] All operations logged to audit log
- [ ] Tested with Entra ID SCIM provisioning
- [ ] Tested with Google Workspace SCIM

## When to Stop and Ask

You MUST stop and ask the PM if:

1. **Supabase Edge Function limits** - If function size or runtime limits are hit
2. **SCIM spec ambiguity** - If SCIM spec interpretation is unclear
3. **IdP-specific requirements** - If Entra ID or Google require non-standard behavior
4. **Filter complexity** - If complex filters beyond userName are needed
5. **User ID mapping** - If external ID generation strategy is unclear
6. **Audit log format** - If SCIM event logging format needs specification
...
### Dependencies
- **EUM-001**: Uses database tables for users and org members
- **EUM-003**: Uses Audit Service for logging SCIM events

### For EUM-017 (Token UI)
- Tokens are stored with hash (one-way)
- Token prefix allows identification
- Last used timestamp for monitoring

### For EUM-020 (Final Integration)
- SCIM server must pass Entra ID compliance testing
- SCIM server must pass Google Workspace testing
- Error responses must match SCIM spec exactly
