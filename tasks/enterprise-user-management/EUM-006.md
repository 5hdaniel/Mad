...
- ❌ Modifying existing individual Microsoft OAuth login
- ❌ Building SSO configuration UI (handled in Phase 3)
- ❌ Implementing SSO Core Service (handled in EUM-008)
- ❌ Creating JIT provisioning service (handled in EUM-009)
- ❌ Implementing SCIM provisioning with Entra ID
- ❌ Supporting Entra ID B2C (consumer accounts)
- ❌ Managing Entra ID app registrations via API
...
- [ ] `getAuthorizationUrl()` generates valid MSAL authorization URL with PKCE
- [ ] `exchangeCodeForTokens()` exchanges code for valid access/ID tokens
- [ ] Token claims are correctly parsed (sub, oid, email, name, tid, groups)
- [ ] `verifyEmailDomain()` correctly validates email against org domains
- [ ] `testConfiguration()` validates tenant/client configuration
- [ ] `getUserInfoFromGraph()` retrieves user profile from Microsoft Graph
- [ ] `getUserGroups()` retrieves group memberships from Graph
- [ ] Client secrets are encrypted before storage
- [ ] Tenant ID mismatch is detected and rejected
...
1. **MSAL version issues** - If `@azure/msal-node` has breaking changes or version conflicts
2. **Token claims differences** - If Entra ID token claims don't match expected structure
3. **Graph API permissions** - If additional Graph permissions are needed beyond specified
4. **Enterprise app registration** - If multi-tenant app registration is needed
5. **Conditional access** - If Entra ID conditional access policies affect authentication
6. **Token storage** - If you need to implement token caching beyond PKCE flow

Do NOT guess or make assumptions. Ask first.

## Integration Notes

### For EUM-008 (SSO Core Service)
- SSO Core will call this service for Entra ID authentication
- Must decrypt client secret before passing to MSAL
- Return standard `EntraIdSSOResult` format

### For EUM-009 (JIT Provisioning)
- Provide `EntraIdUserInfo` structure compatible with JIT service
- Include group memberships for potential role mapping

### For EUM-010 (Integration Task)
- Service must be a singleton
- All errors should be caught and returned in result object
- Setup documentation must be complete

### Shared Concerns
- PKCE verifier storage needs coordination across services
- Redirect URI must be configurable via environment
