...
class EnhancedExportService {
    ...
    async exportTransaction(transaction, communications, options = {}) {
        const { contentType = 'both', exportFormat = 'pdf', representationStartDate, closingDate, } = options;
        try {
            ...
            let filteredComms = this._filterCommunicationsByDate(communications, representationStartDate, closingDate);
            ...
            filteredComms = this._filterByAddressRelevance(filteredComms, transaction.property_address);
            ...
            console.log(`[Enhanced Export] Filtered to ${filteredComms.length} communications (verified address relevance)`);
    ...
    _filterByAddressRelevance(communications, propertyAddress) {
        ...
        return communications.filter((comm) => {
            ...
            const hasAddressReference = addressParts.some((part) => {
                if (part.length < 3)
                    return false; // Skip very short parts
            ...
            console.log(`[Enhanced Export] Filtered out email (no address match): "${comm.subject}" from ${comm.sender}`);
            return false;
        });
    }
    /**
    ...
    async _exportPDF(transaction, communications) {
        ...
        const fileName = this._sanitizeFileName(`Transaction_${transaction.property_address}_${Date.now()}.pdf`);
        const outputPath = path_1.default.join(downloadsPath, fileName);
        return await pdfExportService_1.default.generateTransactionPDF(transaction, communications, outputPath);
    }
    /**
    ...
    async _exportCSV(transaction, communications, format) {
        ...
        const fileName = this._sanitizeFileName(`Transaction_${transaction.property_address}_${Date.now()}.${ext}`);
        const outputPath = path_1.default.join(downloadsPath, fileName);
        // Create CSV content
        const headers = [
            'Date',
        ...
        const csvLines = [
            ...
            `Representation Start: ${transaction.representation_start_date
        ...
        await promises_1.default.writeFile(outputPath, csvContent, 'utf8');
        return outputPath;
    }
    /**
     * Export as JSON
    ...
    async _exportJSON(transaction, communications) {
        const downloadsPath = electron_1.app.getPath('downloads');
        const fileName = this._sanitizeFileName(`Transaction_${transaction.property_address}_${Date.now()}.json`);
        const outputPath = path_1.default.join(downloadsPath, fileName);
        const exportData = {
            transaction: {
                id: transaction.id,
        ...
        await promises_1.default.writeFile(outputPath, JSON.stringify(exportData, null, 2), 'utf8');
        return outputPath;
    }
    /**
    ...
    async _exportTxtEml(transaction, communications) {
        const downloadsPath = electron_1.app.getPath('downloads');
        const folderName = this._sanitizeFileName(`${transaction.property_address}_Export_${Date.now()}`);
        const basePath = path_1.default.join(downloadsPath, folderName);
        // Create folder structure
        await promises_1.default.mkdir(basePath, { recursive: true });
        const emailsPath = path_1.default.join(basePath, 'emails');
        const textsPath = path_1.default.join(basePath, 'texts');
        await promises_1.default.mkdir(emailsPath, { recursive: true });
        await promises_1.default.mkdir(textsPath, { recursive: true });
        ...
        for (let i = 0; i < emails.length; i++) {
            ...
            const emlFileName = this._sanitizeFileName(`${i + 1}_${email.subject || 'no_subject'}.eml`);
            await promises_1.default.writeFile(path_1.default.join(emailsPath, emlFileName), emlContent, 'utf8');
        }
        ...
        for (let i = 0; i < texts.length; i++) {
            ...
            const txtFileName = this._sanitizeFileName(`${i + 1}_${new Date(text.sent_at).toISOString().split('T')[0]}.txt`);
            await promises_1.default.writeFile(path_1.default.join(textsPath, txtFileName), txtContent, 'utf8');
        }
        // Create summary.txt
        const summaryContent = this._createSummary(transaction, communications);
        await promises_1.default.writeFile(path_1.default.join(basePath, 'SUMMARY.txt'), summaryContent, 'utf8');
        return basePath;
    }
    /**
    ...
    _createEMLContent(email) {
        ...
        if (email.recipients)
            lines.push(`To: ${email.recipients}`);
        if (email.cc)
            lines.push(`Cc: ${email.cc}`);
        if (email.bcc)
            lines.push(`Bcc: ${email.bcc}`);
        lines.push(`Subject: ${email.subject || '(No Subject)'}`);
        lines.push(`Date: ${email.sent_at ? new Date(email.sent_at).toUTCString() : 'Unknown'}`);
    ...
    _createTextContent(text) {
        ...
        lines.push(`Date: ${text.sent_at ? new Date(text.sent_at).toLocaleString() : 'Unknown'}`);
        lines.push('');
    ...
    _createSummary(transaction, communications) {
        ...
        lines.push('');
        lines.push(`Representation Start Date: ${transaction.representation_start_date
            ? new Date(transaction.representation_start_date).toLocaleDateString()
            : 'N/A'}`);
        lines.push(`Closing Date: ${transaction.closing_date
            ? new Date(transaction.closing_date).toLocaleDateString()
            : 'N/A'}`);
        lines.push('');
        lines.push(`Sale Price: ${transaction.sale_price
        ...
        lines.push(`  - Emails: ${communications.filter((c) => c.communication_type === 'email').length}`);
        lines.push(`  - Texts: ${communications.filter((c) => c.communication_type === 'text').length}`);
        lines.push('');
...
exports.default = new EnhancedExportService();
