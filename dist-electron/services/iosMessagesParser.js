...
exports.iOSMessagesParser = void 0;
exports.convertAppleTimestamp = convertAppleTimestamp;
...
class iOSMessagesParser {
     ...
     */
    open(backupPath) {
        const dbPath = path_1.default.join(backupPath, iOSMessagesParser.SMS_DB_HASH);
        try {
            this.db = new better_sqlite3_multiple_ciphers_1.default(dbPath, { readonly: true });
            this.backupPath = backupPath;
            electron_log_1.default.info('iOSMessagesParser: Opened database', { backupPath });
        }
        catch (error) {
            electron_log_1.default.error('iOSMessagesParser: Failed to open database', {
                backupPath,
    ...
    close() {
        if (this.db) {
            try {
                this.db.close();
                electron_log_1.default.info('iOSMessagesParser: Closed database');
            }
            catch (error) {
                electron_log_1.default.error('iOSMessagesParser: Error closing database', {
                    error: error instanceof Error ? error.message : String(error)
                });
            }
            this.db = null;
    ...
    getHandle(handleId) {
      ...
      `).get(handleId);
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting handle', {
                handleId,
                error: error instanceof Error ? error.message : String(error)
            });
            return '';
    ...
    getConversations() {
      ...
      `).all();
          ...
          `).get(chat.ROWID);
                    ...
                    electron_log_1.default.error('iOSMessagesParser: Error processing chat', {
                        chatId: chat.ROWID,
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting conversations', {
                error: error instanceof Error ? error.message : String(error)
            });
            return [];
        }
    ...
    getParticipants(chatId) {
      ...
      `).all(chatId);
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting participants', {
                chatId,
                error: error instanceof Error ? error.message : String(error)
            });
            return [];
    ...
    getMessages(chatId, limit, offset) {
      ...
      `;
            ...
            const rows = this.db.prepare(query).all(chatId);
            return rows.map(row => this.mapMessage(row));
        }
        catch (error) {
            electron_log_1.default.error('iOSMessagesParser: Error getting messages', {
                chatId,
                limit,
                offset,
                error: error instanceof Error ? error.message : String(error)
    ...
    getAttachments(messageId) {
      ...
      `).all(messageId);
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting attachments', {
                messageId,
                error: error instanceof Error ? error.message : String(error)
            });
            return [];
    ...
    searchMessages(query, limit) {
      ...
      `;
            ...
            const rows = this.db.prepare(sql).all(searchPattern);
            return rows.map(row => this.mapMessage(row));
        }
        catch (error) {
            electron_log_1.default.error('iOSMessagesParser: Error searching messages', {
                error: error instanceof Error ? error.message : String(error)
            });
            return [];
        }
    ...
    getMessageCount(chatId) {
      ...
      `).get(chatId);
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting message count', {
                chatId,
                error: error instanceof Error ? error.message : String(error)
            });
            return 0;
     ...
     */
    getConversationWithMessages(chatId, limit, offset) {
        this.ensureOpen();
        try {
      ...
      `).get(chatId);
            ...
            electron_log_1.default.error('iOSMessagesParser: Error getting conversation with messages', {
                chatId,
                error: error instanceof Error ? error.message : String(error)
            });
            return null;
...
exports.iOSMessagesParser = iOSMessagesParser;
// The sms.db hash in iOS backups (SHA-1 of domain + path)
iOSMessagesParser.SMS_DB_HASH = '3d0d7e5fb2ce288813306e4d4636395e047a3d28';
exports.default = iOSMessagesParser;
