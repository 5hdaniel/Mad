...
class TransactionExtractorService {
     ...
     * @param email - Email object from Gmail/Outlook
    ...
    _isRealEstateRelated(text) {
        const keywordCount = this.keywords.transaction.filter(keyword => text.includes(keyword.toLowerCase())).length;
    ...
    _detectTransactionType(text) {
        const purchaseScore = this.keywords.purchase.filter(keyword => text.includes(keyword.toLowerCase())).length;
        const saleScore = this.keywords.sale.filter(keyword => text.includes(keyword.toLowerCase())).length;
    ...
    _calculateConfidence(text) {
        ...
        const keywordMatches = this.keywords.transaction.filter(keyword => text.includes(keyword.toLowerCase())).length;
        ...
        const contextMatches = contextPhrases.filter(phrase => text.includes(phrase.toLowerCase())).length;
        if (contextMatches > 0) {
    ...
    _analyzeSubjectLine(text) {
        ...
        if (this.patterns.address.test(firstLine))
            bonus += 5;
        ...
        switch (fieldName) {
            case 'closing_date':
                ...
                if (allMatches.length === 1)
                    confidence += 20; // Only one date = likely correct
                if (allMatches.length <= 3)
                    confidence += 10;
                if (this._isReasonableClosingDate(extractedValue))
                    confidence += 40;
                break;
            case 'sale_price':
                // Higher confidence if:
            ...
            case 'property_address':
                ...
                // - Has zip code
                if (this.patterns.address.test(extractedValue))
                    confidence += 50;
                if (extractedValue.match(/\d{5}(-\d{4})?/))
                    confidence += 20; // Has zip
    ...
    _extractParties(email, text) {
        ...
        if (email.from)
            parties.push({ email: email.from, role: 'sender' });
        if (email.to) {
    ...
    generateTransactionSummary(emailGroup) {
        ...
        const avgConfidence = Math.round(emailGroup.reduce((sum, e) => sum + e.confidence, 0) / emailGroup.length);
...
exports.default = new TransactionExtractorService();
