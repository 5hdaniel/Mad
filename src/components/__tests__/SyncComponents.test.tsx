...
describe('SyncProgressBar', () => {
  describe('Rendering', () => {
    ...
    it('should clamp progress to 0-100 range', () => {
      const { rerender } = render(<SyncProgressBar percent={-10} phase="Test" />);
      expect(screen.getByText('0%')).toBeInTheDocument();
    ...
    it('should apply animation class when animated is true', () => {
      const { container } = render(<SyncProgressBar percent={50} phase="Test" animated={true} />);
      const progressFill = container.querySelector('.progress-fill');
      expect(progressFill).toHaveClass('animate-pulse');
    });

    it('should not apply animation class when animated is false', () => {
      const { container } = render(<SyncProgressBar percent={50} phase="Test" animated={false} />);
      const progressFill = container.querySelector('.progress-fill');
...
describe('SyncStatus', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should show reminder message', () => {
      ...
      expect(screen.getByText('Keep your iPhone connected and unlocked')).toBeInTheDocument();
    });
...
describe('SyncComplete', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should use singular minute for 1 minute duration', () => {
      ...
      render(<SyncComplete result={oneMinuteResult} onContinue={mockOnContinue} />);
...
describe('SyncError', () => {
  ...
  describe('Rendering', () => {
    it('should render error title', () => {
      render(<SyncError error="DEVICE_DISCONNECTED" onRetry={mockOnRetry} onCancel={mockOnCancel} />);
    ...
    it('should display friendly error for DEVICE_DISCONNECTED', () => {
      ...
      expect(screen.getByText('Your iPhone was disconnected. Please reconnect and try again.')).toBeInTheDocument();
    });

    it('should display friendly error for DEVICE_LOCKED', () => {
      render(<SyncError error="DEVICE_LOCKED" onRetry={mockOnRetry} onCancel={mockOnCancel} />);

      expect(screen.getByText('Please unlock your iPhone and try again.')).toBeInTheDocument();
    });

    it('should display friendly error for BACKUP_FAILED', () => {
      render(<SyncError error="BACKUP_FAILED" onRetry={mockOnRetry} onCancel={mockOnCancel} />);

      expect(screen.getByText('The sync could not be completed. Please try again.')).toBeInTheDocument();
    });

    it('should display friendly error for PASSWORD_INCORRECT', () => {
      render(<SyncError error="PASSWORD_INCORRECT" onRetry={mockOnRetry} onCancel={mockOnCancel} />);

      expect(screen.getByText('The backup password was incorrect. Please try again.')).toBeInTheDocument();
    });

    it('should display generic error for unknown error codes', () => {
      render(<SyncError error="UNKNOWN_ERROR" onRetry={mockOnRetry} onCancel={mockOnCancel} />);
...
describe('CancelSyncModal', () => {
  ...
  describe('Rendering', () => {
    it('should not render when isOpen is false', () => {
      render(<CancelSyncModal isOpen={false} onConfirm={mockOnConfirm} onCancel={mockOnCancel} />);

      expect(screen.queryByText('Cancel Sync?')).not.toBeInTheDocument();
    });

    it('should render when isOpen is true', () => {
      render(<CancelSyncModal isOpen={true} onConfirm={mockOnConfirm} onCancel={mockOnCancel} />);
    ...
    it('should display warning message', () => {
      ...
      expect(screen.getByText(/sync is still in progress/i)).toBeInTheDocument();
  ...
  describe('Interactions', () => {
    it('should call onConfirm when Cancel Sync button clicked', () => {
      ...
      const cancelButton = screen.getAllByText('Cancel Sync').find(
