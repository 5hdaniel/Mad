...
describe('KeychainExplanation', () => {
  ...
  describe('Full Explanation Mode', () => {
    it('should render the full explanation screen by default', () => {
      ...
      expect(screen.getByText('Protect your data with Keychain')).toBeInTheDocument();
    });

    it('should show returning user text when hasPendingLogin is false', () => {
      render(<KeychainExplanation onContinue={mockOnContinue} hasPendingLogin={false} />);

      expect(
        screen.getByText(/Magic Audit needs to access your Mac's Keychain to decrypt your local database/i)
      ).toBeInTheDocument();
    });

    it('should show new user text when hasPendingLogin is true', () => {
      render(<KeychainExplanation onContinue={mockOnContinue} hasPendingLogin={true} />);

      expect(
        screen.getByText(/Magic Audit needs to set up secure storage on your Mac to protect your data/i)
      ).toBeInTheDocument();
    });
    ...
    it('should show "Don\'t show again" checkbox', () => {
      ...
      expect(screen.getByText(/Don't show this explanation again/i)).toBeInTheDocument();
  ...
  describe('Loading State', () => {
    it('should show loading spinner when isLoading is true', () => {
      render(<KeychainExplanation onContinue={mockOnContinue} isLoading={true} />);

      expect(screen.getByText(/Please enter your password in the system dialog/i)).toBeInTheDocument();
    });
  ...
  describe('Simple Waiting Mode (skipExplanation)', () => {
    it('should show simple waiting screen when skipExplanation is true', () => {
      ...
      expect(screen.getByText(/Click continue to enter your Mac password/i)).toBeInTheDocument();
    });
