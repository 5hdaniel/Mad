...
interface DriversAPI {
  checkApple: () => Promise<{ installed: boolean; version?: string; serviceRunning: boolean; error?: string }>;
  hasBundled: () => Promise<{ hasBundled: boolean }>;
  installApple: () => Promise<{ success: boolean; cancelled?: boolean; error?: string; rebootRequired?: boolean }>;
  openITunesStore: () => Promise<{ success: boolean; error?: string }>;
  checkUpdate?: () => Promise<{ updateAvailable: boolean; installedVersion: string | null; bundledVersion: string | null }>;
}
...
type InstallStatus = 'checking' | 'not-installed' | 'needs-update' | 'installing' | 'installed' | 'error' | 'cancelled';
        ...
        if (driverResult.installed && driverResult.serviceRunning) {
          ...
          if (drivers.checkUpdate) {
            try {
              ...
              console.warn('[AppleDriverSetup] Could not check for updates:', updateError);
            }
          }
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStatus('cancelled');
        setErrorMessage('Installation was cancelled. You can try again or skip for now.');
      } else {
        setStatus('error');
        setErrorMessage(result.error || 'Installation failed. Please try again or install iTunes manually.');
      }
    } catch (error) {
      setStatus('error');
      ...
      <div className="flex-1 flex items-start justify-center pt-8 px-4">
        <div className="max-w-xl w-full">
          ...
          <div className="text-center mb-6">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${
              status === 'installed' ? 'bg-green-100' :
              status === 'needs-update' ? 'bg-amber-100' :
              status === 'error' || status === 'cancelled' ? 'bg-red-100' :
              'bg-blue-100'
            }`}>
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">
              {status === 'checking' ? 'Checking System...' :
               status === 'installing' ? 'Installing Tools...' :
               status === 'installed' ? 'Tools Installed!' :
               status === 'needs-update' ? 'Update Available' :
               status === 'error' || status === 'cancelled' ? 'Installation Issue' :
               'Install iPhone Tools'}
            </h1>

            <p className="text-gray-600">
              {status === 'checking' ? 'Checking if Apple tools are already installed...' :
               status === 'installing' ? 'Please approve the installation when prompted...' :
               status === 'installed' ? 'Your computer is ready to sync with your iPhone.' :
               status === 'needs-update' ? 'A newer version of Apple tools is available.' :
              ...
              <div className="bg-amber-50 rounded-lg p-4 mb-6">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Current version: <strong>{installedVersion || 'Unknown'}</strong></span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">•</span>
                    <span>New version: <strong>{bundledVersion || 'Available'}</strong></span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800">
                    <p className="font-medium mb-1">Administrator Permission Required</p>
                    <p>
                      Updating will require administrator permission. Your existing settings will be preserved.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {hasBundled ? (
                ...
                <>
                  <p className="text-sm text-amber-600 mb-3 text-center">
                    Bundled update not found. You can update via iTunes from the Microsoft Store.
                  </p>
                  <button
            ...
            <>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">•</span>
                    <span><strong>Apple Mobile Device Support</strong> - Required to communicate with your iPhone</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-500 mt-0.5">•</span>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800">
                    <p className="font-medium mb-1">Administrator Permission Required</p>
                    <p>
                      By clicking "Install Tools", you consent to installing Apple Mobile Device Support on your computer.
                      Windows will ask for administrator permission to proceed.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {hasBundled ? (
                ...
                <>
                  <p className="text-sm text-amber-600 mb-3 text-center">
                    Bundled installer not found. Install iTunes to get the required drivers.
                  </p>
                  <button
                    onClick={handleOpenITunesStore}
                    className="w-full py-3 px-4 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 transition-all shadow-md hover:shadow-lg mb-3"
        ...
        {status === 'not-installed' && (
          <p className="text-xs text-gray-400 text-center mt-4">
            Magic Audit does not distribute Apple software. We help you install Apple's official tools
            which are required to communicate with iPhone devices.
          </p>
        )}
        </div>
      </div>
