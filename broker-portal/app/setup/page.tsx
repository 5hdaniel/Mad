 ...
 * Microsoft uses consent prompt to ensure email/profile scopes are granted.
 */
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  unsupported_provider: 'Organization setup requires a Google or Microsoft work account.',
  no_tenant: 'Could not verify your Microsoft organization. Please contact support.',
  no_email:
    'Could not retrieve your email address. Please check your account permissions and try again.',
  provision_failed: 'Failed to create your organization. Please try again or contact support.',
  consumer_account: 'Personal accounts are not supported. Please use your work account.',
};
...
function SetupForm() {
  ...
  const handleSetup = async (provider: 'google' | 'azure') => {
    const { createClient } = await import('@/lib/supabase/client');
    const supabase = createClient();
    setLoading(provider);
    setError(null);
    ...
    const { error: authError } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        queryParams: {
          prompt: provider === 'azure' ? 'consent' : 'select_account',
        },
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-500">
            Sign in with a work account from the organization you want to set up.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span>{loading === 'google' ? 'Setting up...' : 'Set up with Google'}</span>
          </button>

          <button
            onClick={() => handleSetup('azure')}
            disabled={loading !== null}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {loading === 'azure' ? (
            ...
            <span>{loading === 'azure' ? 'Setting up...' : 'Set up with Microsoft'}</span>
          </button>
        </div>
