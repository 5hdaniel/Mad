...
The app now includes an automatic rebuild script. Simply run:
```bash
npm install
```

The `postinstall` script will automatically rebuild sqlite3 for your architecture.
...
Or to rebuild all native modules:
```bash
npm run rebuild:all
```
