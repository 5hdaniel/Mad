...
interface TransactionContactInput {
  contact_id: string;
  role?: string;
...
class DatabaseService {
  ...
  private async _runAdditionalMigrations(): Promise<void> {
    ...
    try {
      ...
      const userColumns = await this._all<TableInfo>(`PRAGMA table_info(users_local)`);
      ...
      const transactionColumns = await this._all<TableInfo>(`PRAGMA table_info(transactions)`);
      ...
      const tcColumns = await this._all<TableInfo>(`PRAGMA table_info(transaction_contacts)`);
      ...
      const verifyTcColumns = await this._all<TableInfo>(`PRAGMA table_info(transaction_contacts)`);
      ...
      const contactColumns = await this._all<TableInfo>(`PRAGMA table_info(contacts)`);
   ...
   * @private
   */
  private _run(sql: string, params: any[] = []): Promise<DatabaseRunResult> {
    return new Promise((resolve, reject) => {
  ...
  async getUserById(userId: string): Promise<User | undefined> {
    ...
    return await this._get<User>(sql, [userId]);
  }
  ...
  async getUserByEmail(email: string): Promise<User | undefined> {
    ...
    return await this._get<User>(sql, [email]);
  }

  /**
   * Get user by OAuth provider and ID
   */
  async getUserByOAuthId(provider: OAuthProvider, oauthId: string): Promise<User | undefined> {
    const sql = 'SELECT * FROM users_local WHERE oauth_provider = ? AND oauth_id = ?';
    return await this._get<User>(sql, [provider, oauthId]);
  }
   ...
   * @param termsVersion - Version of Terms of Service being accepted
   * @param privacyVersion - Version of Privacy Policy being accepted
   */
   ...
   */
  async createContact(userId: string, contactData: CreateContactInput): Promise<Contact | undefined> {
    const id = crypto.randomUUID();
    const sql = `
  ...
  async getContactById(contactId: string): Promise<Contact | undefined> {
    ...
    return await this._get<Contact>(sql, [contactId]);
  }
   ...
   * @param userId - User ID
   * @param propertyAddress - Optional property address to rank contacts who communicated about it
   * @returns Contacts with metadata about recent communication and address relevance
   */
  async getContactsSortedByActivity(userId: string, propertyAddress: string | null = null): Promise<ContactWithActivity[]> {
    let sql = `
      SELECT
        c.*,
   ...
   */
  async updateContact(contactId: string, updates: Partial<Contact>): Promise<Contact | undefined> {
   ...
   */
  async getTransactionsByContact(contactId: string): Promise<TransactionWithRole[]> {
    const transactions: TransactionWithRole[] = [];
    const transactionMap = new Map<string, Omit<TransactionWithRole, 'roles'> & { roles: string[] }>();
    ...
    const directResults = await this._all<any>(directQuery, [
    ...
    const junctionResults = await this._all<any>(junctionQuery, [contactId]);
    ...
    try {
      ...
      const jsonResults = await this._all<any>(jsonQuery, [contactId]);
      ...
      const fallbackResults = await this._all<any>(fallbackQuery, [`%"${contactId}"%`]);
   ...
   */
  async getOrCreateContactFromEmail(userId: string, email: string, name: string | null = null): Promise<Contact> {
   ...
   */
  async saveOAuthToken(userId: string, provider: OAuthProvider, purpose: OAuthPurpose, tokenData: OAuthTokenInput): Promise<string> {
    const id = crypto.randomUUID();
   ...
   * Get OAuth token
   */
  async getOAuthToken(userId: string, provider: OAuthProvider, purpose: OAuthPurpose): Promise<OAuthToken | undefined> {
    const sql = `
    ...
    return token as OAuthToken | undefined;
  }
  ...
  async updateOAuthToken(tokenId: string, updates: Partial<OAuthToken>): Promise<void> {
    ...
    Object.keys(updates).forEach((key) => {
      if (allowedFields.includes(key)) {
        let value = updates[key as keyof OAuthToken];
   ...
   */
  async createTransaction(userId: string, transactionData: CreateTransactionInput): Promise<string> {
    const id = crypto.randomUUID();
  ...
  async getTransactionsByUserId(userId: string): Promise<Transaction[]> {
    ...
    `;
    return await this._all<Transaction>(sql, [userId]);
  }
  ...
  async getTransactionById(transactionId: string): Promise<Transaction | undefined> {
    ...
    return await this._get<Transaction>(sql, [transactionId]);
  }
  ...
  async updateTransaction(transactionId: string, updates: Partial<Transaction>): Promise<Transaction | undefined> {
    ...
    Object.keys(updates).forEach((key) => {
      if (allowedFields.includes(key)) {
        let value = updates[key as keyof Transaction];
   ...
   */
  async saveCommunication(userId: string, communicationData: CommunicationInput): Promise<string> {
    const id = crypto.randomUUID();
   ...
   */
  async getCommunicationsByTransactionId(transactionId: string): Promise<Communication[]> {
    const sql = `
   ...
   */
  async getCommunicationsByUserId(userId: string, limit: number = 100): Promise<Communication[]> {
    const sql = `
      SELECT * FROM communications
      WHERE user_id = ?
      ORDER BY sent_at DESC
      LIMIT ?
    `;
    return await this._all<Communication>(sql, [userId, limit]);
  }
   ...
   */
  async saveExtractedData(transactionId: string, fieldName: string, fieldValue: string, sourceCommId: string | null, confidence: number | null): Promise<string> {
    const id = crypto.randomUUID();
   ...
   */
  async assignContactToTransaction(transactionId: string, data: TransactionContactInput): Promise<string> {
    const id = crypto.randomUUID();
   ...
   */
  async getTransactionContacts(transactionId: string): Promise<TransactionContactWithDetails[]> {
    const sql = `
      SELECT
        tc.*,
    ...
    return await this._all<TransactionContactWithDetails>(sql, [transactionId]);
  }

  /**
   * Get all contacts for a specific role in a transaction
   */
  async getTransactionContactsByRole(transactionId: string, role: string): Promise<TransactionContactWithDetails[]> {
    const sql = `
      SELECT
        tc.*,
        c.name as contact_name,
    ...
    return await this._all<TransactionContactWithDetails>(sql, [transactionId, role]);
  }

  /**
   * Update contact role information
   */
  async updateContactRole(transactionId: string, contactId: string, updates: Partial<TransactionContact>): Promise<void> {
    ...
    Object.keys(updates).forEach((key) => {
      if (allowedFields.includes(key)) {
        fields.push(`${key} = ?`);
        values.push(updates[key as keyof TransactionContact]);
      }
    });
   ...
   */
  async submitFeedback(userId: string, feedbackData: UserFeedbackInput): Promise<string> {
    const id = crypto.randomUUID();
   ...
   */
  async getFeedbackForTransaction(transactionId: string): Promise<UserFeedback[]> {
    const sql = `
   ...
   */
  async getExtractionMetrics(userId: string, fieldName: string | null = null): Promise<ExtractionMetrics[]> {
    let sql = `
    ...
    return await this._all<ExtractionMetrics>(sql, params);
  }
   ...
   * @private
   */
  private async _updateExtractionMetrics(userId: string, feedbackData: UserFeedbackInput): Promise<void> {
    ...
    const existing = await this._get<ExtractionMetrics>(
    ...
    if (existing) {
      ...
      const updates: Partial<ExtractionMetrics> = {
