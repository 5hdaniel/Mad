...
import { MIN_CONTACT_RECORD_COUNT, CONTACTS_BASE_DIR, DEFAULT_CONTACTS_DB } from '../constants';

/**
 * Contact map result type
 */
...
/**
 * Get contact names result
 */
interface GetContactNamesResult {
...
/**
 * Person map indexed by person_id
 */
interface PersonMap {
  [person_id: number]: PersonMapEntry;
}
 ...
 *
 * @returns contactMap: Maps phone/email to contact name
 ...
 */
export async function getContactNames(): Promise<GetContactNamesResult> {
    ...
    try {
      ...
      for (const dbPath of dbFiles) {
        ...
        try {
          ...
          const dbAll = promisify(db.all.bind(db)) as (sql: string) => Promise<RecordCount[]>;
 ...
 *
 * @param contactsDbPath - Path to the contacts database
 * @returns contactMap: Maps phone/email to name, phoneToContactInfo: Maps phone to full contact info
 */
export async function loadContactsFromDatabase(contactsDbPath: string): Promise<{
  contactMap: ContactMap;
  phoneToContactInfo: PhoneToContactInfo;
}> {
  const contactMap: ContactMap = {};
  ...
  try {
    ...
    console.error(`[ContactsService] Cannot access database at ${contactsDbPath}:`, (error as Error).message);
 ...
 *
 * @param contactsResult - Contact records from database
 * @param phonesResult - Phone numbers from database
 * @param emailsResult - Email addresses from database
 * @returns Person map indexed by person_id
 */
function buildPersonMap(
  contactsResult: ContactRecord[],
  phonesResult: PhoneRecord[],
  emailsResult: EmailRecord[]
): PersonMap {
 ...
 *
 * @param personMap - Person map indexed by person_id
 * @param contactMap - Output: Maps phone/email to name
 * @param phoneToContactInfo - Output: Maps phone to full contact info
 */
 ...
 *
 * @param contactId - Phone number or email from message
 * @param chatIdentifier - Chat identifier from Messages
 * @param displayName - Display name from Messages (if available)
 * @param contactMap - Contact lookup map
 * @returns Resolved contact name or formatted identifier
 */
