...
class SupabaseService {
   ...
   */
  async syncUser(userData: UserData): Promise<CloudUser> {
   ...
   * @returns User data
   */
  async getUserById(userId: string): Promise<CloudUser> {
    this._ensureInitialized();
   ...
   */
  async syncTermsAcceptance(userId: string, termsVersion: string, privacyVersion: string): Promise<CloudUser> {
    this._ensureInitialized();
  ...
  async checkDeviceLimit(userId: string, deviceId: string): Promise<DeviceLimitResult> {
    ...
    try {
      ...
      const isCurrentDevice = devices?.some((d: any) => d.device_id === deviceId) || false;
