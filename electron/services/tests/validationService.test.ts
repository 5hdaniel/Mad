...
describe('ValidationService', () => {
  describe('validateUserId', () => {
    ...
    it('should throw ValidationError for invalid UUID', () => {
      expect(() => ValidationService.validateUserId('invalid-uuid')).toThrow(ValidationError);
      expect(() => ValidationService.validateUserId('invalid-uuid')).toThrow('Invalid UUID format');
    });
    ...
    it('should throw ValidationError for non-string', () => {
      expect(() => ValidationService.validateUserId(123)).toThrow(ValidationError);
      expect(() => ValidationService.validateUserId(null)).toThrow(ValidationError);
    });
  });
  ...
  describe('validateEmail', () => {
    ...
    it('should reject invalid email formats', () => {
      ...
      invalidEmails.forEach(email => {
        expect(() => ValidationService.validateEmail(email)).toThrow(ValidationError);
      });
    });

    it('should reject emails exceeding 255 characters', () => {
      const longEmail = 'a'.repeat(250) + '@example.com';
      expect(() => ValidationService.validateEmail(longEmail)).toThrow('Email exceeds maximum length');
    });
  });
  ...
  describe('validatePhone', () => {
    it('should validate phone numbers', () => {
      ...
      validPhones.forEach(phone => {
        expect(() => ValidationService.validatePhone(phone, true)).not.toThrow();
      });
    });
    ...
    it('should throw ValidationError when required but empty', () => {
      expect(() => ValidationService.validatePhone('', true)).toThrow(ValidationError);
      expect(() => ValidationService.validatePhone(null, true)).toThrow('Phone number is required');
    });
    ...
    it('should reject invalid phone formats', () => {
      ...
      invalidPhones.forEach(phone => {
        expect(() => ValidationService.validatePhone(phone, true)).toThrow(ValidationError);
      });
    });
  });
  ...
  describe('validateZipCode', () => {
    ...
    it('should throw ValidationError for invalid ZIP codes', () => {
      expect(() => ValidationService.validateZipCode('1234', false)).toThrow(ValidationError);
      expect(() => ValidationService.validateZipCode('abcde', false)).toThrow(ValidationError);
    });
  });
  ...
  describe('validateUserData', () => {
    ...
    it('should require email for new users', () => {
      ...
      expect(() => ValidationService.validateUserData(userData, true)).toThrow('Email is required');
    });

    it('should validate OAuth provider', () => {
      const userData = { email: 'test@example.com', oauth_provider: 'invalid' };
      expect(() => ValidationService.validateUserData(userData)).toThrow(ValidationError);
    });

    it('should validate subscription tier', () => {
      const validData = { email: 'test@example.com', subscription_tier: 'free' };
      expect(() => ValidationService.validateUserData(validData)).not.toThrow();

      const invalidData = { email: 'test@example.com', subscription_tier: 'invalid' };
      expect(() => ValidationService.validateUserData(invalidData)).toThrow(ValidationError);
    });
  });
  ...
  describe('validateTransactionData', () => {
    it('should validate complete transaction data', () => {
      ...
      const validated = ValidationService.validateTransactionData(transactionData);
      ...
      expect(validated.longitude).toBe(-74.0060);
    });

    it('should require property address for new transactions', () => {
      const transactionData = { transaction_type: 'purchase' };
      expect(() => ValidationService.validateTransactionData(transactionData, true))
    ...
    it('should validate transaction type', () => {
      ...
      expect(() => ValidationService.validateTransactionData(validData)).not.toThrow();

      const invalidData = {
        property_address: '123 Main St',
        transaction_type: 'invalid'
      };
      expect(() => ValidationService.validateTransactionData(invalidData)).toThrow(ValidationError);
    });
    ...
    it('should validate coordinates', () => {
      ...
      expect(() => ValidationService.validateTransactionData(invalidLat)).toThrow('Invalid latitude');
      ...
      expect(() => ValidationService.validateTransactionData(invalidLng)).toThrow('Invalid longitude');
    });
    ...
    it('should accept valid coordinates', () => {
      ...
      expect(() => ValidationService.validateTransactionData(validCoords)).not.toThrow();
    });
  });
  ...
  describe('validateContactData', () => {
    ...
    it('should require name for new contacts', () => {
      ...
      expect(() => ValidationService.validateContactData(contactData, true))
        .toThrow('Name is required');
    });

    it('should validate source', () => {
      const validData = { name: 'John Doe', source: 'gmail' };
      expect(() => ValidationService.validateContactData(validData)).not.toThrow();

      const invalidData = { name: 'John Doe', source: 'invalid' };
      expect(() => ValidationService.validateContactData(invalidData)).toThrow(ValidationError);
    });
    ...
    it('should trim and validate name length', () => {
      ...
      expect(() => ValidationService.validateContactData(tooLong)).toThrow(ValidationError);
    });
  });
  ...
  describe('validateProvider', () => {
    ...
    it('should reject invalid providers', () => {
      expect(() => ValidationService.validateProvider('facebook')).toThrow(ValidationError);
      expect(() => ValidationService.validateProvider('')).toThrow(ValidationError);
      expect(() => ValidationService.validateProvider(null)).toThrow(ValidationError);
    });
  });
