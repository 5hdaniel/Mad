...
export class ValidationService {
  ...
  static validateTransactionData(
    ...
    if (isNew || transactionData.property_address !== undefined) {
      ...
      if (transactionData.property_address) {
        if (typeof transactionData.property_address !== "string") {
          throw new ValidationError(
            "Property address must be a string",
            "property_address",
          );
        }
        ...
        if (trimmed.length < 5 || trimmed.length > 500) {
          throw new ValidationError(
            "Property address length must be between 5 and 500",
            "property_address",
          );
        }
    ...
    if (
      transactionData.latitude !== undefined ||
      transactionData.longitude !== undefined
    ) {
  ...
  static validateContactData(contactData: any, isNew: boolean = false): any {
    if (!contactData || typeof contactData !== "object") {
      throw new ValidationError(
        "Contact data must be an object",
        "contactData",
      );
    }
    ...
    if (isNew || contactData.name !== undefined) {
      ...
      if (contactData.name) {
        ...
        if (trimmed.length === 0 || trimmed.length > 200) {
          throw new ValidationError(
            "Name length must be between 1 and 200",
            "name",
          );
        }
