...
(mockNotificationConstructor as any).isSupported = jest.fn().mockReturnValue(true);
...
jest.mock('electron', () => ({
  app: {
    getPath: jest.fn((pathType: string) => {
      if (pathType === 'exe') return '/Applications/MagicAudit.app/Contents/MacOS/MagicAudit';
      return '/mock/path';
    }),
  },
  shell: {
...
jest.mock('child_process', () => ({
  exec: jest.fn((cmd: string, callback: Function) => {
    if (mockExecShouldFail) {
      callback(mockExecError || new Error('exec failed'), '', '');
    } else {
      callback(null, '', '');
    }
  }),
...
describe('MacOSPermissionHelper', () => {
  ...
  describe('requestContactsPermission', () => {
    ...
    it('should handle AppleScript execution failure', async () => {
      ...
      mockExecError = new Error('osascript failed');
  ...
  describe('setupFullDiskAccess', () => {
    it('should open System Preferences to Full Disk Access', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toBe('System Preferences opened to Full Disk Access');
  ...
  describe('openPrivacyPane', () => {
    ...
    it('should open Accessibility privacy pane', async () => {
      const result = await macOSPermissionHelper.openPrivacyPane('accessibility');
    ...
    it('should use custom pane ID if not in predefined list', async () => {
      const result = await macOSPermissionHelper.openPrivacyPane('Privacy_CustomPane');
  ...
  describe('checkFullDiskAccessStatus', () => {
    ...
    it('should handle ENOENT error (file not found)', async () => {
      const accessError = new Error('ENOENT: no such file') as NodeJS.ErrnoException;
  ...
  describe('runPermissionSetupFlow', () => {
    ...
    it('should report partial success when contacts permission fails', async () => {
      mockExecShouldFail = true;
      mockExecError = new Error('Contacts failed');
