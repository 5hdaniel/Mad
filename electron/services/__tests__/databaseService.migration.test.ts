...
describe("DatabaseService Migration Robustness (TASK-2048)", () => {
  ...
  describe("dry-run mode", () => {
    it("should return migration plan without executing", async () => {
      ...
      expect(plan).toEqual({
        ...
        pendingMigrations: [
          ...
          {
            version: 36,
            description: expect.stringContaining("oauth_provider"),
          },
        ],
        wouldRunCount: 7,
      });
    ...
    it("should report zero pending when all applied", async () => {
      ...
      // Setup: version = 36 (all applied including TASK-1337 oauth_provider migration)
      mockStatement.get
        .mockReturnValueOnce({ name: "schema_version" })
        .mockReturnValueOnce({ version: 36 });
  ...
  describe("happy path", () => {
    it("should run all pending migrations in sequence", async () => {
      ...
      // Transaction should have been called seven times (for migrations 30, 31, 32, 33, 34, 35, and 36)
    ...
    it("should skip already-applied migrations", async () => {
      ...
      // version = 36, all migrations applied (including TASK-1337 oauth_provider)
      mockStatement.get
        .mockReturnValueOnce({ name: "schema_version" })
        .mockReturnValueOnce({ version: 36 });
