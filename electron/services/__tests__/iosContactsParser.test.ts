...
import type { Database as _Database, Statement as _Statement } from 'better-sqlite3';
...
const mockMultiValues = [
  ...
  { record_id: 1, property: 3, label: '_$!<Mobile>!$_', value: '(555) 123-4567' },
  { record_id: 1, property: 4, label: '_$!<Home>!$_', value: 'john.doe@example.com' },
  // Jane Smith's multiple phones
  { record_id: 2, property: 3, label: '_$!<Work>!$_', value: '+1 555 987 6543' },
  ...
  { record_id: 3, property: 4, label: '_$!<Work>!$_', value: 'info@testcompany.com' },
...
const createMockDatabase = () => {
  ...
  return {
    prepare: jest.fn((sql: string) => {
      ...
      } else if (sql.includes('FROM ABMultiValue') && sql.includes('ORDER BY mv.record_id')) {
        // All multi-values query
        stmt.all.mockImplementation(() => mockMultiValues);
      } else if (sql.includes('FROM ABPerson') && sql.includes('WHERE ROWID = ?')) {
        // Single contact by ID
        stmt.get.mockImplementation((id: number) =>
          mockContacts.find(c => c.ROWID === id) || null
        );
      } else if (sql.includes('FROM ABMultiValue') && sql.includes('WHERE mv.record_id = ?')) {
