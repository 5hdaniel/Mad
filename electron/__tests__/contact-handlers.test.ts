...
const mockContactsService = { getContactNames: getContactNames as jest.MockedFunction<typeof getContactNames> };
...
describe('Contact Handlers', () => {
  ...
  describe('contacts:get-all', () => {
    it('should return all imported contacts for user', async () => {
      ...
      mockDatabaseService.getImportedContactsByUserId.mockResolvedValue(mockContacts);
  ...
  describe('contacts:get-available', () => {
    it('should return available contacts for import', async () => {
      mockContactsService.getContactNames.mockResolvedValue({
        phoneToContactInfo: {
          '555-1234': { name: 'John Doe', phones: ['555-1234'], emails: ['john@example.com'] },
          '555-5678': { name: 'Jane Smith', phones: ['555-5678'], emails: ['jane@example.com'] },
        },
        status: 'loaded',
      });
  ...
  describe('contacts:import', () => {
    ...
    it('should import contacts successfully', async () => {
      mockDatabaseService.createContact.mockImplementation(async (data: any) => ({
    ...
    it('should reject more than 1000 contacts', async () => {
      const manyContacts = Array(1001).fill({ name: 'Test', email: 'test@example.com' });
  ...
  describe('contacts:get-sorted-by-activity', () => {
    it('should return contacts sorted by activity', async () => {
      const sortedContacts = [
        ...
        { id: 'contact-2', name: 'Less Active Jane', lastActivity: new Date(Date.now() - 86400000) },
      ];
      mockDatabaseService.getContactsSortedByActivity.mockResolvedValue(sortedContacts);
      ...
      expect(result.success).toBe(true);
      expect(result.contacts).toHaveLength(2);
      expect(mockDatabaseService.getContactsSortedByActivity).toHaveBeenCalledWith(
        TEST_USER_ID,
        '123 Main St'
      );
    });
  ...
  describe('contacts:update', () => {
    ...
    it('should update contact successfully', async () => {
      ...
      const result = await handler(mockEvent, TEST_CONTACT_ID, { name: 'New Name' });
  ...
  describe('contacts:checkCanDelete', () => {
    ...
    it('should return false when contact has transactions', async () => {
      ...
      mockDatabaseService.getTransactionsByContact.mockResolvedValue(transactions);
  ...
  describe('contacts:remove', () => {
    it('should remove contact from local database successfully', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockDatabaseService.removeContact).toHaveBeenCalledWith(TEST_CONTACT_ID);
    });
