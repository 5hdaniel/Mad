...
describe("fileUtils", () => {
  ...
  describe("sanitizeFilenamePreserveCase", () => {
    ...
    it("should replace spaces when preserveSpaces is false", () => {
      expect(sanitizeFilenamePreserveCase("Test File", false)).toBe(
        "Test_File",
      );
    });

    it("should remove special characters", () => {
      expect(sanitizeFilenamePreserveCase("Test@File#Name!")).toBe(
        "Test_File_Name_",
      );
    });
  ...
  describe("ensureUniqueFilename", () => {
    ...
    it("should use correct extension", async () => {
      ...
      const result = await ensureUniqueFilename(
        "document",
        "docx",
        checkExists,
      );
