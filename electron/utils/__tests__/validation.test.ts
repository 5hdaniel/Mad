...
describe('Transaction Validation', () => {
  describe('validateTransactionId', () => {
    ...
    it('should reject non-string values', () => {
      ...
      expect(() => validateTransactionId(123)).toThrow('Transaction ID must be a string');
    });

    it('should reject invalid UUID format', () => {
      expect(() => validateTransactionId('not-a-uuid')).toThrow(ValidationError);
      expect(() => validateTransactionId('not-a-uuid')).toThrow('Transaction ID must be a valid UUID');
    });

    it('should handle required parameter', () => {
      expect(() => validateTransactionId(null, true)).toThrow('Transaction ID is required');
  ...
  describe('validateTransactionData - Date Fields', () => {
    ...
    it('should reject invalid date formats for representation_start_date', () => {
      ...
      expect(() => validateTransactionData(data, true)).toThrow(ValidationError);
      expect(() => validateTransactionData(data, true)).toThrow('Representation start date must be in YYYY-MM-DD format');
    });
    ...
    it('should reject invalid date formats for closing_date', () => {
      ...
      expect(() => validateTransactionData(data, true)).toThrow(ValidationError);
      expect(() => validateTransactionData(data, true)).toThrow('Closing date must be in YYYY-MM-DD format');
    });
    ...
    it('should reject invalid closing_date_verified values', () => {
      ...
      expect(() => validateTransactionData(data, true)).toThrow(ValidationError);
      expect(() => validateTransactionData(data, true)).toThrow('Closing date verified must be 0 or 1');
    });
  ...
  describe('validateTransactionData - Price Fields', () => {
    ...
    it('should reject negative prices', () => {
      ...
      expect(() => validateTransactionData(data, true)).toThrow(ValidationError);
      expect(() => validateTransactionData(data, true)).toThrow('Sale price must be a non-negative number');
    });
    ...
    it('should reject invalid price values', () => {
      ...
      expect(() => validateTransactionData(data, true)).toThrow(ValidationError);
      expect(() => validateTransactionData(data, true)).toThrow('Listing price must be a non-negative number');
    });
  });
