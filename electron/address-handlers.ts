...
export const registerAddressHandlers = (): void => {
  ...
  ipcMain.handle('address:initialize', async (event: IpcMainInvokeEvent, apiKey?: string): Promise<AddressResponse> => {
    try {
      // Validate API key
  ...
  ipcMain.handle('address:get-suggestions', async (event: IpcMainInvokeEvent, input: string, sessionToken?: string): Promise<AddressResponse> => {
    try {
      // Validate input
  ...
  ipcMain.handle('address:get-details', async (event: IpcMainInvokeEvent, placeId: string): Promise<AddressResponse> => {
    try {
      // Validate place ID
  ...
  // Geocode an address
  ipcMain.handle('address:geocode', async (event: IpcMainInvokeEvent, address: string): Promise<AddressResponse> => {
    try {
      // Validate address
  ...
  // Validate an address
  ipcMain.handle('address:validate', async (event: IpcMainInvokeEvent, address: string): Promise<AddressResponse> => {
    try {
      // Validate address
      const validatedAddress = validateString(address, 'address', {
        required: true,
